    ...
    var x = $.post('/pcp', function(data){
        ...
        for (i in columns) {
          ...
          "Strong Family-Community Ties Rating","Trust Rating","Student Achievement Rating"].indexOf(col) != -1){
            ...
            var category_sort = data_col.map(function(d){return d.key});
            category_sort.sort(function(a,b){return a.localeCompare(b)});

            yScale[col] = d3.scalePoint()
            .domain(category_sort.map(function(d){return d}))
            .range([innerHeight, 0]);
          }
          else{
        ...
        var pcp_g = g.selectAll(".dimension")
            ...
            .call(d3.drag()
              ...
              .on("drag", function(d) {
                ...
                columns.sort(function(a, b) { return position(a) - position(b); });
                xScale.domain(columns.map(function(d) { return d; }));
                pcp_g.attr("transform", function(d) { return "translate(" + position(d) + ")"; })
              })
              .on("end", function(d) {
                delete dragging[d];
